...
MODEL_PATH = './flan-t5-gec--finetune-model'
...
def run_evaluation():
    ...
    for line in source_lines:
        line = line.strip()
        if line:
            prefixed_lines.append(TASK_PREFIX + line)

    print(f"Loaded {len(prefixed_lines)} sentences to correct.")

    all_predictions = []
    print(f"Generating corrections in batches of {INFERENCE_BATCH_SIZE}...")

    with torch.no_grad():
        for i in tqdm(range(0, len(prefixed_lines), INFERENCE_BATCH_SIZE), desc="Generating corrections"):
            
            batch_lines = prefixed_lines[i:i + INFERENCE_BATCH_SIZE]
